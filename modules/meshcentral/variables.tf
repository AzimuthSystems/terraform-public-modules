...
variable "r53_config" {
  type = map(string)
  default = {
    zone_id = "EXAMPLE_ZONE_ID" # Replace with your Route53 Hosted Zone ID
    type = "A" # Choose the dns record type (A, CNAME, ALIAS, ETC)
    ttl = "60" # Time To Live - 60 for testing.  Production should be a higher value to limit updates and query cost.
  }
}
